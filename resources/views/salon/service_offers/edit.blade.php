     ...
     <div class="card">
        ...
        @include('includes.msg')
          @if(isset($approved)&& $approved==0)
             <p style="color:red"> Your offer is waiting for approval.</p>
             @endif
        {!! Form::open(["url"=>env('ADMIN_URL')."/salon/services/offers/edit","id"=>"main","method"=>"post",'files'=> true]) !!}
            @csrf
        ...
        <div class="col-sm-10">
            {!!Form::text("start_date",$offer->start_date,["class"=>"form-control datepicker","id"=>"date", "aria-describedby"=>"emailHelp","placeholder"=>"Choose From Date","autocomplete"=>"off",'required' => 'required']) !!}
            <span class="messages"></span>
        ...
        <div class="col-sm-10">
            {!! Form::text("end_date",$offer->end_date,[ "placeholder"=>"Choose To Date","class"=>"form-control datepicker","id"=>"date", "aria-describedby"=>"emailHelp",'required' => 'required']) !!}
            <span class="messages"></span>
        </div>
        </div>
